        ...
        <div>
            ...
            {voices.map((voice)=>(
                <div key={voice.voice_id} className="voiceR">
                    ...
                    {/* a button with each voice sample so that when clicked it will set the audiopreview url state with the current url */}
                   <button key={voice.voice_id} onClick={()=>setAudioPreviewUrl(voice.preview_audio)} style={{margin:"2px"}}>▶️{voice.name} ({voice.gender})</button>
